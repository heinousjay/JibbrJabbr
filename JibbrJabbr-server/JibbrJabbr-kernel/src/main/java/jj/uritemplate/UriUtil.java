...
/**
 * <p>
 * A light-weight utility class for applying encoding to values that are applied
...
public final class UriUtil {
	...
	static {
		...
		/**
		 * Allowed chars in Fragments (#). See:
		 * http://tools.ietf.org/html/rfc3986#appendix-A
		 * 
		 * The definition in http://tools.ietf.org/html/rfc6570#section-3.2.1
		 * (URI-Template-RFC) is not correct! Not all characters(#[]) in
		 * GEN_DELIM are allowed here!
		 * 
		 * fragment = *( pchar / "/" / "?" ) pchar = unreserved / sub-delims /
		 * ":" / "@"
		 */
		...
		/**
		 * This is a special uri encoding for Uri-Templates, allowing Reserved
	...
	/**
	 * Encodes fragments, allowing only the characters defined in uri
	 * fragments(http://tools.ietf .org/html/rfc3986#appendix-A). This are a few
	 * less then in the reserved expression.
	 * 
	 * @param sourceValue
	 *            should not contain a PCT encoded string.
	...
	/**
	 * Encodes the reserved expression, allowing all reserved and unreserved
	 * characters.
	 * 
	 * @param sourceValue
	 *            should not contain a PCT encoded string.
	...
	/**
	 * Uses a white list to encode all characters that are not contained in this
	 * list.
	 */
		...
		for (int i = 0; i < source.length; i++) {
			...
			if (allowedCharacters.get(c & 0xff)) {
				...
				char hex1 = Character.toUpperCase(Character.forDigit(
						(c >> 4) & 0xF, 16));
				char hex2 = Character.toUpperCase(Character.forDigit(c & 0xF,
						16));
				out.write(hex1);
				out.write(hex2);
			}
