...
/**
 * implements a RFC 6570 uri template 
 * @author jason
 *
 */
public class UriTemplate {
	 ...
	 * method will create a new {@link UriTemplate} from the base and copy the
	 * variables from the base template to the new {@link UriTemplate}.
	 * </p>
	 * <p>
	 * This method is useful when the base template is less volatile than the
	 * child expression and you want to merge the two.
	 * </p>
	 * 
	 * @param base
	 * @return
	...
	/**
	 * Expands the given expression string using the variable replacements in
	 * the supplied {@link Map}.
	 * 
	 * @param expression
	 * @param values
	 * @since 1.0
	...
	*/
	...
	private static boolean containsOperator(String op) {
	...
	public UriTemplate(final String template) {
		templateBuffer = new StringBuilder(template);
	}
	
	public UriTemplate(final UriTemplate base) {
		templateBuffer = new StringBuilder(base.getTemplate());
	...
	public String expand() {
		...
		// this is a stringbuffer because they never bothered updating
		// the matcher to use appendable at the API. not sure why since
		// it would have been totally compatible. oh well.
			...
			if (values.containsKey(varSpec.getVariableName())) {
				...
				if (explodable) {
					...
					if (value instanceof VarExploder) {
						...
						exploder = new VarExploder(value);
					}
				...
				if (value instanceof Date) {
					value = DateFormatHelper.uriFormat((Date) value);
				}
	 ...
	 * <li>an instance of {@link Number}
	 * <li>
	 * <li>an instance of {@link Date}
	 * <li>
	 * </ul>
	 * 
	 * @param value
	...
	/**
	 * Check to ensure that the values being passed down do not contain nested
	 * data structures.
	 * 
	 * @param obj
	 */
	...
	private String expandMap(Operator operator, VarSpec varSpec, Map<String, Object> variable) {
		...
		if (varSpec.getModifier() != Modifier.EXPLODE && 
			(operator == Operator.MATRIX ||
			 ...
			 operator == Operator.CONTINUATION)
		) {
