...
class NextConnectionFunction extends BaseFunction implements HostObject, ContributesScript {
	...
	public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
		...
		Object obj = context.webSocketConnectionHost().scope().get(PROP_CURRENT_ITERATOR, scope);
		if (!(obj instanceof Iterator<?>)) {
			// something really went wrong up in here
			log.error("broadcast attempted but the current iterator does not exist for {}", context.webSocketConnectionHost());
		...
		if (next != null) {
			...
			context.webSocketConnectionHost().scope().delete(PROP_CURRENT_ITERATOR);
			context.webSocketConnectionHost().scope().delete(PROP_ITERATOR_NEEDS_FINISH);
		}
